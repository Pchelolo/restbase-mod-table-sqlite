...
dbu.makeSchemaInfo = function makeSchemaInfo(schema) {
    ...
    psi.allSecondaryIndexKeys.add(psi.tid);
    Object.keys(psi.secondaryIndexes).forEach(function(indexName) {
        ...
        Object.keys(psi.secondaryIndexes[indexName].attributes)
        .filter(function(key) {
            return psi.secondaryIndexes[indexName].iKeyMap[key]
                && (psi.secondaryIndexes[indexName].iKeyMap[key].type === 'hash'
                    || psi.secondaryIndexes[indexName].iKeyMap[key].type === 'range');
        })
...
dbu.buildGetQuery = function(tableName, query, schema, includePreparedForDelete) {
    ...
    if (query.index) {
        ...
        sql = 'select ' + proj + ' from [' + tableName + '_secondaryIndex] natural join ['  + tableName + '_data]'
