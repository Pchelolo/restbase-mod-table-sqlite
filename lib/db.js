...
DB.prototype.dropTable = function(domain, bucket) {
    ...
    var deleteRequest = function(schema) {
        ...
        if (secondaryIndexNames.length > 0) {
            queries.push({ sql: 'drop table ' + dbu.createName(tableName + '_secondaryIndex') });
        }
...
DB.prototype._get = function(tableName, req, schema, includePreparedForDelete) {
    ...
    return self.client.all(
        dbu.buildGetQuery(tableName, req, schema, includePreparedForDelete),
        dbu.extractGetParams(req, schema, includePreparedForDelete)
